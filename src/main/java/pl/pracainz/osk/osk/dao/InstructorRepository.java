...
public interface InstructorRepository extends JpaRepository<Instructor, Integer> {
	...
	// lista wykładów
	@Query("SELECT l FROM Lecture l " + "JOIN Course c ON c.id = l.course "
			+ "JOIN Instructor i ON i.id = c.instructor " + "WHERE i.id = :id " + "ORDER BY l.date ")
	List<Lecture> findLectures(@Param("id") int id);

	// lista egzaminow
	@Query("SELECT ie FROM InternalExam ie " + "JOIN Instructor i ON i.id = ie.instructor "
			+ "WHERE i.id = :id AND ie.deleted = 0 " + "ORDER BY ie.dateHour DESC")
	List<InternalExam> queryFindExams(@Param("id") int id);

	@Query("SELECT ie FROM InternalExam ie " + "JOIN Instructor i ON i.id = ie.instructor "
			+ "WHERE i.id = :id AND ie.deleted = 1" + "ORDER BY ie.dateHour DESC")
	List<InternalExam> findArchivedExams(@Param("id") int id);

	// lista kursantów
	@Query("SELECT DISTINCT s FROM Student s " + "JOIN Driving d ON d.student = s.id "
			+ "JOIN Timetable t ON d.timetable = t.id " + "JOIN Instructor i ON i.id = t.instructor "
			+ "WHERE i.id = :id")
	List<Student> queryFindStudents(@Param("id") int id);

	// lista opinii
	@Query("SELECT io FROM InstructorOpinion io " + "JOIN Instructor i ON i.id = io.instructor "
			+ "WHERE i.id = :id AND io.status='zatwierdzona'")
